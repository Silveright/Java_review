...
public class FileInputStreamExample1 {
	...
	public static void main(String[] args) {
		if(args.length<1) {
			System.out.println("파일명을 Arguments탭에 output1.dat입력해주세요.");
			return;
		}
		...
		try {
			...
			while(true) {
				...
				//대문자(X가 대문자)로 출력하며 숫자가 한 자리이면 앞자리를 0으로 채운다.
				//%02x: 2자리 16진수(%X)로 출력하되,
				//소문자(x가 소문자)로 출력하며 숫자가 한 자리이면 앞자리를 0으로 채운다.
